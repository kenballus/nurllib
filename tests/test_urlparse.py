...
class UrlParseTestCase(unittest.TestCase):
    ...
    def checkRoundtrips(self, url, parsed, split):
        ...
        result = parse_module.urlsplit(url)
        self.assertSequenceEqual(result, split)
        t = (result.scheme, result.netloc, result.path,
             result.query, result.fragment)
        self.assertSequenceEqual(t, split)
        result2 = parse_module.urlunsplit(result)
        self.assertSequenceEqual(result2, url)
        self.assertSequenceEqual(result2, result.geturl())

        # check the fixpoint property of re-parsing the result of geturl()
        result3 = parse_module.urlsplit(result.geturl())
        self.assertEqual(result3.geturl(), result.geturl())
        self.assertSequenceEqual(result3, result)
        self.assertEqual(result3.scheme,   result.scheme)
        self.assertEqual(result3.netloc,   result.netloc)
    ...
    def test_qsl(self):
        for orig, expect in parse_qsl_test_cases:
            result = parse_module.parse_qsl(orig, keep_blank_values=True)
            self.assertEqual(result, expect, "Error parsing %r" % orig)
            expect_without_blanks = [v for v in expect if len(v[1])]
            result = parse_module.parse_qsl(orig, keep_blank_values=False)
            self.assertEqual(result, expect_without_blanks,
                            "Error parsing %r" % orig)

    def test_qs(self):
        for orig, expect in parse_qs_test_cases:
            result = parse_module.parse_qs(orig, keep_blank_values=True)
            self.assertEqual(result, expect, "Error parsing %r" % orig)
            expect_without_blanks = {v: expect[v]
                                     for v in expect if len(expect[v][0])}
            result = parse_module.parse_qs(orig, keep_blank_values=False)
            self.assertEqual(result, expect_without_blanks,
                            "Error parsing %r" % orig)
    ...
    def test_http_roundtrips(self):
        # parse_module.urlsplit treats 'http:' as an optimized special case,
    ...
    def checkJoin(self, base, relurl, expected):
        ...
        self.assertEqual(parse_module.urljoin(base, relurl), expected)
        bytes_components = baseb, relurlb, expectedb = [
                            x.encode('ascii') for x in str_components]
        self.assertEqual(parse_module.urljoin(baseb, relurlb), expectedb)

    def test_unparse_parse(self):
        str_cases = ['Python', './Python','x-newscheme://foo.com/stuff','x://y','x:/y','x:/','/',]
        bytes_cases = [x.encode('ascii') for x in str_cases]
        for u in str_cases + bytes_cases:
            self.assertEqual(parse_module.urlunsplit(parse_module.urlsplit(u)), u)
            self.assertEqual(parse_module.urlunparse(parse_module.urlparse(u)), u)
    ...
    def test_RFC2368(self):
        ...
        self.assertEqual(parse_module.urlparse('mailto:1337@example.org'),
    ...
    def test_RFC2732(self):
        ...
        for url, hostname, port in str_cases + bytes_cases:
            urlparsed = parse_module.urlparse(url)
        ...
        for invalid_url in str_cases + bytes_cases:
            self.assertRaises(ValueError, parse_module.urlparse, invalid_url)
    ...
    def test_urldefrag(self):
        ...
        for url, defrag, frag in str_cases + bytes_cases:
            result = parse_module.urldefrag(url)
    ...
    def test_urlsplit_scoped_IPv6(self):
        p = parse_module.urlsplit('http://[FE80::822a:a8ff:fe49:470c%tESt]:1234')
        self.assertEqual(p.hostname, "fe80::822a:a8ff:fe49:470c%tESt")
        self.assertEqual(p.netloc, '[FE80::822a:a8ff:fe49:470c%tESt]:1234')

        p = parse_module.urlsplit(b'http://[FE80::822a:a8ff:fe49:470c%tESt]:1234')
    ...
    def test_urlsplit_remove_unsafe_bytes(self):
        ...
        scheme = "ht\ntp"
        for _ in range(2):
            p = parse_module.urlsplit(url, scheme=scheme)
            self.assertEqual(p.scheme, "http")
            self.assertEqual(p.geturl(), "http://www.python.org/javascript:alert('msg')/?query=something#fragment")
    ...
    def test_urlsplit_strip_url(self):
        ...
        p = parse_module.urlsplit(noise.decode("utf-8") + query_spaces_url)
        ...
        p = parse_module.urlsplit("www.pypi.org ")
        ...
        self.assertEqual(parse_module.urlunsplit(p), "www.pypi.org ")
    ...
    def test_attributes_bad_port(self):
        ...
        for bytes in (False, True):
            for parse in (parse_module.urlsplit, parse_module.urlparse):
    ...
    def test_noslash(self):
        ...
        self.assertEqual(parse_module.urlparse("http://example.com?blahblah=/foo"),
                         ('http', 'example.com', '', '', 'blahblah=/foo', ''))
        self.assertEqual(parse_module.urlparse(b"http://example.com?blahblah=/foo"),
    ...
    def test_withoutscheme(self):
        ...
        self.assertEqual(parse_module.urlparse("path"),
                ('','','path','','',''))
        self.assertEqual(parse_module.urlparse("//www.python.org:80"),
                ('','www.python.org:80','','','',''))
        self.assertEqual(parse_module.urlparse("http://www.python.org:80"),
                ('http','www.python.org:80','','','',''))
        # Repeat for bytes input
        self.assertEqual(parse_module.urlparse(b"path"),
                (b'',b'',b'path',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"//www.python.org:80"),
                (b'',b'www.python.org:80',b'',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"http://www.python.org:80"),
                (b'http',b'www.python.org:80',b'',b'',b'',b''))

    def test_portseparator(self):
        # Issue 754016 makes changes for port separator ':' from scheme separator
        self.assertEqual(parse_module.urlparse("http:80"), ('http','','80','','',''))
        self.assertEqual(parse_module.urlparse("https:80"), ('https','','80','','',''))
        self.assertEqual(parse_module.urlparse("path:80"), ('path','','80','','',''))
        self.assertEqual(parse_module.urlparse("http:"),('http','','','','',''))
        self.assertEqual(parse_module.urlparse("https:"),('https','','','','',''))
        self.assertEqual(parse_module.urlparse("http://www.python.org:80"),
                ('http','www.python.org:80','','','',''))
        # As usual, need to check bytes input as well
        self.assertEqual(parse_module.urlparse(b"http:80"), (b'http',b'',b'80',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"https:80"), (b'https',b'',b'80',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"path:80"), (b'path',b'',b'80',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"http:"),(b'http',b'',b'',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"https:"),(b'https',b'',b'',b'',b'',b''))
        self.assertEqual(parse_module.urlparse(b"http://www.python.org:80"),
                (b'http',b'www.python.org:80',b'',b'',b'',b''))

    def test_usingsys(self):
        # Issue 3314: sys module is used in the error
        self.assertRaises(TypeError, parse_module.urlencode, "foo")

    def test_anyscheme(self):
        # Issue 7904: s3://foo.com/stuff has netloc "foo.com".
        self.assertEqual(parse_module.urlparse("s3://foo.com/stuff"),
                         ('s3', 'foo.com', '/stuff', '', '', ''))
        self.assertEqual(parse_module.urlparse("x-newscheme://foo.com/stuff"),
                         ('x-newscheme', 'foo.com', '/stuff', '', '', ''))
        self.assertEqual(parse_module.urlparse("x-newscheme://foo.com/stuff?query#fragment"),
                         ('x-newscheme', 'foo.com', '/stuff', '', 'query', 'fragment'))
        self.assertEqual(parse_module.urlparse("x-newscheme://foo.com/stuff?query"),
                         ('x-newscheme', 'foo.com', '/stuff', '', 'query', ''))

        # And for bytes...
        self.assertEqual(parse_module.urlparse(b"s3://foo.com/stuff"),
                         (b's3', b'foo.com', b'/stuff', b'', b'', b''))
        self.assertEqual(parse_module.urlparse(b"x-newscheme://foo.com/stuff"),
                         (b'x-newscheme', b'foo.com', b'/stuff', b'', b'', b''))
        self.assertEqual(parse_module.urlparse(b"x-newscheme://foo.com/stuff?query#fragment"),
                         (b'x-newscheme', b'foo.com', b'/stuff', b'', b'query', b'fragment'))
        self.assertEqual(parse_module.urlparse(b"x-newscheme://foo.com/stuff?query"),
                         (b'x-newscheme', b'foo.com', b'/stuff', b'', b'query', b''))

    def test_default_scheme(self):
        # Exercise the scheme parameter of urlparse() and urlsplit()
        for func in (parse_module.urlparse, parse_module.urlsplit):
    ...
    def test_parse_fragments(self):
        ...
        for url, attr, expected_frag in tests:
            for func in (parse_module.urlparse, parse_module.urlsplit):
                if attr == "params" and func is parse_module.urlsplit:
                    attr = "path"
    ...
    def test_mixed_types_rejected(self):
        ...
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlparse("www.python.org", b"http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlparse(b"www.python.org", "http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlsplit("www.python.org", b"http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlsplit(b"www.python.org", "http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlunparse(( b"http", "www.python.org","","","",""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlunparse(("http", b"www.python.org","","","",""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlunsplit((b"http", "www.python.org","","",""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urlunsplit(("http", b"www.python.org","","",""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urljoin("http://python.org", b"http://python.org")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse_module.urljoin(b"http://python.org", "http://python.org")
    ...
    def test_result_pairs(self):
        ...
        result_types = [
          parse_module.DefragResult,
          parse_module.SplitResult,
          parse_module.ParseResult,
        ]
        for result_type in result_types:
            self._check_result_type(result_type)

    def test_parse_qs_encoding(self):
        result = parse_module.parse_qs("key=\u0141%E9", encoding="latin-1")
        self.assertEqual(result, {'key': ['\u0141\xE9']})
        result = parse_module.parse_qs("key=\u0141%C3%A9", encoding="utf-8")
        self.assertEqual(result, {'key': ['\u0141\xE9']})
        result = parse_module.parse_qs("key=\u0141%C3%A9", encoding="ascii")
        self.assertEqual(result, {'key': ['\u0141\ufffd\ufffd']})
        result = parse_module.parse_qs("key=\u0141%E9-", encoding="ascii")
        self.assertEqual(result, {'key': ['\u0141\ufffd-']})
        result = parse_module.parse_qs("key=\u0141%E9-", encoding="ascii",
                                                          errors="ignore")
        self.assertEqual(result, {'key': ['\u0141-']})
    ...
    def test_parse_qsl_encoding(self):
        result = parse_module.parse_qsl("key=\u0141%E9", encoding="latin-1")
        self.assertEqual(result, [('key', '\u0141\xE9')])
        result = parse_module.parse_qsl("key=\u0141%C3%A9", encoding="utf-8")
        self.assertEqual(result, [('key', '\u0141\xE9')])
        result = parse_module.parse_qsl("key=\u0141%C3%A9", encoding="ascii")
        self.assertEqual(result, [('key', '\u0141\ufffd\ufffd')])
        result = parse_module.parse_qsl("key=\u0141%E9-", encoding="ascii")
        self.assertEqual(result, [('key', '\u0141\ufffd-')])
        result = parse_module.parse_qsl("key=\u0141%E9-", encoding="ascii",
                                                          errors="ignore")
        self.assertEqual(result, [('key', '\u0141-')])
    ...
    def test_parse_qsl_max_num_fields(self):
        with self.assertRaises(ValueError):
            parse_module.parse_qs('&'.join(['a=a']*11), max_num_fields=10)
        parse_module.parse_qs('&'.join(['a=a']*10), max_num_fields=10)
    ...
    def test_parse_qs_separator(self):
        ...
        for orig, expect in parse_qs_semicolon_cases:
            with self.subTest(f"Original: {orig!r}, Expected: {expect!r}"):
                result = parse_module.parse_qs(orig, separator=';')
                self.assertEqual(result, expect, "Error parsing %r" % orig)
                result_bytes = parse_module.parse_qs(orig, separator=b';')
    ...
    def test_parse_qsl_separator(self):
        ...
        for orig, expect in parse_qsl_semicolon_cases:
            with self.subTest(f"Original: {orig!r}, Expected: {expect!r}"):
                result = parse_module.parse_qsl(orig, separator=';')
                self.assertEqual(result, expect, "Error parsing %r" % orig)
                result_bytes = parse_module.parse_qsl(orig, separator=b';')
                self.assertEqual(result_bytes, expect, "Error parsing %r" % orig)
    ...
    def test_urlencode_sequences(self):
        ...
        result = parse_module.urlencode({'a': [1, 2], 'b': (3, 4, 5)}, True)
        ...
        result = parse_module.urlencode({'a': Trivial()}, True)
        self.assertEqual(result, 'a=trivial')

    def test_urlencode_quote_via(self):
        result = parse_module.urlencode({'a': 'some value'})
        self.assertEqual(result, "a=some+value")
        result = parse_module.urlencode({'a': 'some value/another'},
                                        quote_via=parse_module.quote)
        self.assertEqual(result, "a=some%20value%2Fanother")
        result = parse_module.urlencode({'a': 'some value/another'},
                                        safe='/', quote_via=parse_module.quote)
        self.assertEqual(result, "a=some%20value/another")

    def test_quote_from_bytes(self):
        self.assertRaises(TypeError, parse_module.quote_from_bytes, 'foo')
        result = parse_module.quote_from_bytes(b'archaeological arcana')
        self.assertEqual(result, 'archaeological%20arcana')
        result = parse_module.quote_from_bytes(b'')
        self.assertEqual(result, '')
        result = parse_module.quote_from_bytes(b'A'*10_000)
        self.assertEqual(result, 'A'*10_000)
        result = parse_module.quote_from_bytes(b'z\x01/ '*253_183)
        self.assertEqual(result, 'z%01/%20'*253_183)

    def test_unquote_to_bytes(self):
        result = parse_module.unquote_to_bytes('abc%20def')
        self.assertEqual(result, b'abc def')
        result = parse_module.unquote_to_bytes('')
        self.assertEqual(result, b'')

    def test_quote_errors(self):
        self.assertRaises(TypeError, parse_module.quote, b'foo',
                          encoding='utf-8')
        self.assertRaises(TypeError, parse_module.quote, b'foo', errors='strict')

    def test_issue14072(self):
        p1 = parse_module.urlsplit('tel:+31-641044153')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '+31-641044153')
        p2 = parse_module.urlsplit('tel:+31641044153')
        self.assertEqual(p2.scheme, 'tel')
        self.assertEqual(p2.path, '+31641044153')
        # assert the behavior for urlparse
        p1 = parse_module.urlparse('tel:+31-641044153')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '+31-641044153')
        p2 = parse_module.urlparse('tel:+31641044153')
        self.assertEqual(p2.scheme, 'tel')
        self.assertEqual(p2.path, '+31641044153')

    def test_invalid_bracketed_hosts(self):
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[192.0.2.146]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[important.com:8000]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[v123r.IP]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[v12ae]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[v.IP]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[v123.]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[v]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[0439:23af::2309::fae7:1234]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@[0439:23af:2309::fae7:1234:2342:438e:192.0.2.146]/Path?Query')
        self.assertRaises(ValueError, parse_module.urlsplit, 'Scheme://user@]v6a.ip[/Path')

    def test_splitting_bracketed_hosts(self):
        p1 = parse_module.urlsplit('scheme://user@[v6a.ip]/path?query')
        self.assertEqual(p1.hostname, 'v6a.ip')
        self.assertEqual(p1.username, 'user')
        self.assertEqual(p1.path, '/path')
        p2 = parse_module.urlsplit('scheme://user@[0439:23af:2309::fae7%test]/path?query')
        self.assertEqual(p2.hostname, '0439:23af:2309::fae7%test')
        self.assertEqual(p2.username, 'user')
        self.assertEqual(p2.path, '/path')
        p3 = parse_module.urlsplit('scheme://user@[0439:23af:2309::fae7:1234:192.0.2.146%test]/path?query')
    ...
    def test_port_casting_failure_message(self):
        ...
        p1 = parse_module.urlparse('http://Server=sde; Service=sde:oracle')
        with self.assertRaisesRegex(ValueError, message):
            p1.port

        p2 = parse_module.urlsplit('http://Server=sde; Service=sde:oracle')
        with self.assertRaisesRegex(ValueError, message):
            p2.port

    def test_telurl_params(self):
        p1 = parse_module.urlparse('tel:123-4;phone-context=+1-650-516')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '123-4')
        self.assertEqual(p1.params, 'phone-context=+1-650-516')

        p1 = parse_module.urlparse('tel:+1-201-555-0123')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '+1-201-555-0123')
        self.assertEqual(p1.params, '')

        p1 = parse_module.urlparse('tel:7042;phone-context=example.com')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '7042')
        self.assertEqual(p1.params, 'phone-context=example.com')

        p1 = parse_module.urlparse('tel:863-1234;phone-context=+1-914-555')
        self.assertEqual(p1.scheme, 'tel')
        self.assertEqual(p1.path, '863-1234')
        self.assertEqual(p1.params, 'phone-context=+1-914-555')

    def test_Quoter_repr(self):
        quoter = parse_module._Quoter(parse_module._ALWAYS_SAFE)
        self.assertIn('Quoter', repr(quoter))

    def test_clear_cache_for_code_coverage(self):
        parse_module.clear_cache()

    def test_urllib_parse_getattr_failure(self):
        """Test that parse_module.__getattr__() fails correctly."""
        with self.assertRaises(AttributeError):
            unused = parse_module.this_does_not_exist
    ...
    def test_all(self):
        ...
        for name in dir(parse_module):
            if name.startswith('_') or name in undocumented:
                continue
            object = getattr(parse_module, name)
            if getattr(object, '__module__', None) == 'parse_module':
                expected.append(name)
        self.assertCountEqual(parse_module.__all__, expected)
    ...
    def test_urlsplit_normalization(self):
        ...
        parse_module.urlsplit('http://\u30d5\u309a:80')
        with self.assertRaises(ValueError):
            parse_module.urlsplit('http://\u30d5\u309a\ufe1380')
...
class Utility_Tests(unittest.TestCase):
    ...
    def test_splittype(self):
        splittype = parse_module._splittype
    ...
    def test_splithost(self):
        splithost = parse_module._splithost
    ...
    def test_splituser(self):
        splituser = parse_module._splituser
    ...
    def test_splitpasswd(self):
        ...
        splitpasswd = parse_module._splitpasswd
    ...
    def test_splitport(self):
        splitport = parse_module._splitport
    ...
    def test_splitnport(self):
        splitnport = parse_module._splitnport
    ...
    def test_splitquery(self):
        ...
        splitquery = parse_module._splitquery
    ...
    def test_splittag(self):
        splittag = parse_module._splittag
    ...
    def test_splitattr(self):
        splitattr = parse_module._splitattr
    ...
    def test_splitvalue(self):
        ...
        splitvalue = parse_module._splitvalue
    ...
    def test_to_bytes(self):
        result = parse_module._to_bytes('http://www.python.org')
        self.assertEqual(result, 'http://www.python.org')
        self.assertRaises(UnicodeError, parse_module._to_bytes,
                          'http://www.python.org/medi\u00e6val')

    def test_unwrap(self):
        for wrapped_url in ('<URL:scheme://host/path>', '<scheme://host/path>',
                            'URL:scheme://host/path', 'scheme://host/path'):
            url = parse_module.unwrap(wrapped_url)
...
class DeprecationTest(unittest.TestCase):
    ...
    def test_Quoter_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            old_class = parse_module.Quoter
            self.assertIs(old_class, parse_module._Quoter)
        self.assertIn('Quoter will be removed', str(cm.warning))

    def test_splittype_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splittype('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splittype() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splithost_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splithost('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splithost() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splituser_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splituser('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splituser() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitpasswd_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitpasswd('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitpasswd() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitport_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitport('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitport() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitnport_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitnport('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitnport() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitquery_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitquery('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitquery() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splittag_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splittag('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splittag() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitattr_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitattr('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitattr() is deprecated as of 3.8, '
                         'use parse_module.urlparse() instead')

    def test_splitvalue_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.splitvalue('')
        self.assertEqual(str(cm.warning),
                         'parse_module.splitvalue() is deprecated as of 3.8, '
                         'use parse_module.parse_qsl() instead')

    def test_to_bytes_deprecation(self):
        with self.assertWarns(DeprecationWarning) as cm:
            parse_module.to_bytes('')
        self.assertEqual(str(cm.warning),
                         'parse_module.to_bytes() is deprecated as of 3.8')


if __name__ == "__main__":
    
    del DeprecationTest, UrlParseTestCase.test_Quoter_repr, UrlParseTestCase.test_attributes_bad_port, UrlParseTestCase.test_port_casting_failure_message, UrlParseTestCase.test_attributes_bad_scheme, UrlParseTestCase.test_all, UrlParseTestCase.test_urlsplit_scoped_IPv6, UrlParseTestCase.test_urlsplit_strip_url, UrlParseTestCase.test_clear_cache_for_code_coverage, Utility_Tests, UrlParseTestCase.test_urlsplit_normalization
    unittest.main()
